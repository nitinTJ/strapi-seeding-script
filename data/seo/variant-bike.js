import brand from "../starapi_variant.json" assert { type: "json" };
import fs from "fs";
...
console.log(pages.length, "ddddddddd");
const best = {
  "S. No.": 13,
  "Page Type": "Variants Detail Page",
  URLs: "https://bikes.tractorjunction.com/en/royal-enfield-bikes/hunter-350-metro-dapper",
  "Meta Title":
    "${brand} ${model} Metro Dapper Price - Hunter 350 Metro Dapper \nMileage, Review & Images",
  "Meta Description":
    "${brand} ${model} Metro Dapper price starts from 195625 in India. \nCheck out ${brand} ${model} Metro Dapper on-road price, Hunter 350 Metro Dapper Mileage, specification, images & colours at BikeJunction.",
...
const promisses = pages
  ...
  .map((item) => {
    ...
    if (item?.attributes?.brand?.data?.attributes?.name) {
      ...
      const model = item?.attributes?.brand?.data?.attributes?.name;
      const price = getPrice(
        item.attributes.min_price,
        item.attributes.max_price
      );
      const capacity = item?.attributes?.enigne?.displacement;
      const transmission = item?.attributes?.clutchandgear?.transmission;
      const tankCapacity = item?.attributes?.fuel?.fuel_capacity;
      const mileage = item?.attributes?.mileage;
      const power = item.attributes?.enigne?.max_power;
      const body = {
        data: {
          id: item.id,
          faq: [
            {
              question: `Q. What Is The Price Of ${brand} ${model} In India?`,
              answer: `A. ${brand} ${model} electric Bike price in India is  ${price}`,
            },
            {
              question: `Q. What Is The Engine Capacity Of ${brand} ${model}?`,
              answer: `A. ${brand} ${model} engine capacity is ${capacity} CC.`,
            },

            {
              question: ` Q. What Type Of Transmission Is Fitted In ${brand} ${model}?`,
              answer: `A. ${brand} ${model} is designed with a ${transmission} type of transmission.`,
            },
            {
              question: `Q. What Is The Mileage Of ${brand} ${model}?`,
              answer: ` A. ${brand} ${model} mileage is ${mileage} Kmpl .`,
            },
            {
              question: `Q. What Is The Power Of The ${brand} ${model} Engine?`,
              answer: `A. The ${brand} ${model} generates a power of ${power}`,
            },

            {
              question: `Q. What Is The Body Type Of ${brand} ${model}?`,
              answer: `A. Hunter 350 is a Bike bike manufactured by ${brand}.`,
            },
            {
              question: `Q. What Is The Fuel Tank Capacity Of ${brand} ${model}?`,
              answer: `A. ${brand} ${model} fuel tank capacity is ${tankCapacity} L.`,
            },
          ],
          //} seo: {,
          //   metaTitle: title,
          ...
          //   ogDescription: desc,
          //   ogType: best["OG Type"],
          //   canonicalURL: `https://bikes.tractorjunction.com/en/${
          //     item.attributes.brand.data.attributes.slug
          //   }-${item?.attributes?.model?.data?.attributes?.classification?.data?.attributes?.Classifications.toLowerCase()}s/${
          //     item.attributes.slug
          //   }`,
          //   ogUrl: `https://bikes.tractorjunction.com/en/${
          //     item.attributes.brand.data.attributes.slug
          //   }-${item?.attributes?.model?.data?.attributes?.classification?.data?.attributes?.Classifications.toLowerCase()}s/${
          //     item.attributes.slug
          //   }`,
