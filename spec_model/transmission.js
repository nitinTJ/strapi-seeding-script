...
import engine from "./modelSpec/bike_transmission_specifications.json" assert { type: "json" };
...
const data = engine
  .map((item) => {
    ...
    if (d.length && d[0].attributes.standard_variant.data) {
      ...
      console.log(id);
      const { clutch, gear_box, transmission } = item;
      const body = {
        data: {
          clutchandgear: { clutch, gear_box, transmission },
        },
      };
